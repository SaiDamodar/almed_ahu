...
#define BUILD_VERSION "v2.6.3-DCP-ALT"
#define BUILD_DATE "2026-05-30"
#define BUILD_FEATURES "dual-CP 1h alternating + 5min rest, single-CP-at-a-time"
...
const char WIFI_SSID[] = "MHINTERNAL";
const char WIFI_PASSWORD[] = "Mhood@123";
...
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
...

Iw6cmvIYp2a3DXBzhGpX5jHHw92+21e+lpg4lT4vs/8BjFKMboDW
-----END RSA PRIVATE KEY-----
)KEY";
...
const unsigned long CP_SWITCH_INTERVAL_MS = 60UL * 60UL * 1000UL;  // 1 hour of ACTUAL run time per CP
// Dual alternating mode: rotate the active CP only after it has actually RUN for CP_SWITCH_INTERVAL_MS
unsigned long cpActiveRunMs = 0;   // accumulated energized run time of the current active CP (ms)
unsigned long cpRunTickAt   = 0;   // last sample taken while active CP energized (0 = not accumulating)
...
// CP Rest Gap - both CPs stay OFF for this long when handing over from one
// compressor to the other (lets the outdoor unit that just ran for 1 hour rest).
// HOSPITAL SAFETY: also covers compressor pressure equalization.
const unsigned long CP_REST_GAP_MS = 5UL * 60UL * 1000UL;  // 5 minutes rest between CP swaps
unsigned long cpSwitchStartedAt = 0;  // When the rest gap / CP switch was initiated
bool cpSwitchInProgress = false;  // True = currently in the 5-min rest gap
...
const char* SITE = "MotherHood";    // Kaveri Hospital
const char* ROOM = "Lab";         // Burns Ward
const char* AHU  = "AHU 19";            // AHU 1
...
void saveSystemState(){
  ...
  prefs.putULong("cpRunMs", cpActiveRunMs);  // persist accumulated CP run time
...
void restoreSystemState(){
  ...
  if (saveTime > 0 && now < 300000) {
    ...
    cpActiveRunMs = prefs.getULong("cpRunMs", 0);  // resume accumulated CP run time
    cpRunTickAt = 0;
...
void controlCP(float t){
  ...
  float offThresh = tempSet;  // active CP turns OFF when temp <= setpoint
  ...
  if (cpMode == CP_DUAL_AUTO) {
    // ===== DUAL CP ALTERNATING MODE =====
    // Only ONE compressor runs at a time. The active CP is allowed to run for up
    // to CP_SWITCH_INTERVAL_MS (1 hour, wall-clock) as the active unit, then it
    // turns OFF and the system rests for CP_REST_GAP_MS (5 min) before the OTHER
    // CP becomes active. This stops one outdoor unit from carrying all the load.
    // The temperature deadband still decides when the ACTIVE CP actually runs.

    // Safety: this mode never runs both compressors together.
    if (dualCpBothOn) {
      dualCpBothOn = false;
      prefs.putBool("dualCpBothOn", false);
    }

    float onThreshDual = tempSet + TEMP_DEADBAND;  // active CP turns ON at/above this

    // Initialise the rotation timer / active CP on first entry
    if (cpLastSwitchAt == 0) {
      cpLastSwitchAt = now;
      if (cpActive != 1 && cpActive != 2) cpActive = 1;
      prefs.putInt("cpActive", cpActive);
      prefs.putULong("cpLastSwitchAt", cpLastSwitchAt);
    }

    // ----- 5-MINUTE REST GAP (handing over between compressors) -----
    if (cpSwitchInProgress) {
      esp_task_wdt_reset();

      // Make sure NOTHING runs during the rest gap
      if (cpOn)  { cpWrite(false);  cpOn = false;  cpLastOffAt = now; }
      if (cp2On) { cp2Write(false); cp2On = false; cpLastOffAt = now; }

      unsigned long elapsed = now - cpSwitchStartedAt;
      if (elapsed >= CP_REST_GAP_MS) {
        cpSwitchInProgress = false;
        cpLastSwitchAt = now;       // bookkeeping only
        cpActiveRunMs = 0;          // new active CP starts with a fresh run-time budget
        cpRunTickAt = 0;
        prefs.putBool("cpSwitchInProgress", false);
        prefs.putULong("cpLastSwitchAt", cpLastSwitchAt);
        prefs.putULong("cpRunMs", 0);
        motorLogMsg("✅ REST COMPLETE → CP" + String(cpActive) + " now active");
      } else {
        static unsigned long lastRestLog = 0;
        if (now - lastRestLog >= 15000) {
          lastRestLog = now;
          unsigned long remaining = (CP_REST_GAP_MS - elapsed) / 1000;
          motorLogMsg("⏳ Resting outdoor units, CP" + String(cpActive) + " starts in " + String(remaining) + "s...");
        }
        return;  // no compressor runs during the rest gap
      }
    }

    // Accumulate run time ONLY while the active compressor is actually energized
    bool activeEnergized = (cpActive == 1 && cpOn) || (cpActive == 2 && cp2On);
    if (activeEnergized) {
      if (cpRunTickAt != 0) cpActiveRunMs += (now - cpRunTickAt);
      cpRunTickAt = now;
    } else {
      cpRunTickAt = 0;  // pause the run-time clock while the CP is off (temp satisfied)
    }

    // ----- ROTATION after 1 HOUR of ACTUAL RUN TIME -----
    if (cpActiveRunMs >= CP_SWITCH_INTERVAL_MS) {
      esp_task_wdt_reset();
      int oldCp = cpActive;

      // Turn off whichever CP is running
      if (cpOn)  { cpWrite(false);  cpOn = false;  cpLastOffAt = now; }
      if (cp2On) { cp2Write(false); cp2On = false; cpLastOffAt = now; }

      // Hand over to the other compressor after the rest gap
      cpActive = (cpActive == 1) ? 2 : 1;
      cpActiveRunMs = 0;        // reset run-time budget for the next CP
      cpRunTickAt = 0;
      ...
      motorLogMsg("🔄 CP ROTATION: CP" + String(oldCp) + " ran 1h → CP" + String(cpActive) + " after " + String(CP_REST_GAP_MS / 60000) + " min rest");
      return;  // begin the 5-minute rest gap
    }

    // ----- TEMPERATURE CONTROL on the ACTIVE compressor only -----
    if (cpActive == 1) {
      if (cp2On) { cp2Write(false); cp2On = false; cpLastOffAt = now; }  // CP2 stays off
      ...
      } else if (cpOn && t <= offThresh && canTurnOff) {
        cpWrite(false);
        cpOn = false;
        cpLastOffAt = now;
        motorLogMsg("CP1 OFF (temp OK, temp=" + String(t,1) + "°C)");
      }
    } else {
      if (cpOn) { cpWrite(false); cpOn = false; cpLastOffAt = now; }  // CP1 stays off
      ...
      } else if (cp2On && t <= offThresh && canTurnOff) {
        cp2Write(false);
        cp2On = false;
        cpLastOffAt = now;
        motorLogMsg("CP2 OFF (temp OK, temp=" + String(t,1) + "°C)");
      }
    }
...
void onMqttMessageLocal(char* topic, byte* payload, unsigned int len){
  ...
  if (doc.containsKey("cpActive")){
    ...
    if ((newCpActive == 1 || newCpActive == 2) && newCpActive != cpActive) {
      ...
      String switchMsg = "🔄 CP MANUAL SWITCH: CP" + String(oldCp) + " → CP" + String(cpActive) + " (" + String(CP_REST_GAP_MS/1000) + "s rest)";
