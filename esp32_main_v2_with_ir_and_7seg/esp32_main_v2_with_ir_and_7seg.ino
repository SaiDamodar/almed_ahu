...
// ---------- 8-digit 7-segment panel (HT16K33 @ 0x70, shares the sensor I2C bus) ----------
// First four digits are the live room readings: [0-1] temperature, [2-3] humidity.
// When a setpoint arrives — IR remote, local MQTT, AWS app or serial — that slot flips
// to the value being set and holds for SEVENSEG_SET_HOLD_MS, shown with a single trailing
// dot ("22.") to tell a target apart from a live reading. Each change re-arms the hold, so
// holding down +/- keeps the target on screen until the user stops, then it falls back.
// The two slots are independent: adjusting temperature never hides the live humidity.
// Digits [4-5] show SDP810 differential pressure in Pa when that sensor is present.
// Without SDP810 they show run state: ON or OF.
// Digits [6-7] show fan speed: 20 = LOW, 30 = MED, 40 = HIGH, -- = system/fan off.
//
// Setpoint changes are picked up by watching tempSet / humSet, so no main-code path is
// touched: whatever writes those variables gets the display behaviour for free.
// Display-only — every entry point no-ops when the panel is absent, so the AHU control
// path runs identically with or without it wired up.
extern FanSpeed fanSpeed;  // declared with the PWM fan block below
#define SEVENSEG_ADDR 0x70

const uint8_t SEVENSEG_DIGITS = 8;
const unsigned long SEVENSEG_SET_HOLD_MS  = 2500;   // how long a setpoint stays on screen
const unsigned long SEVENSEG_SAMPLE_MS    = 100;    // how often we look for changes
const unsigned long SEVENSEG_KEEPALIVE_MS = 1000;   // redraw even when nothing changed
const unsigned long SEVENSEG_PROBE_MS     = 10000;  // retry interval while panel is missing

// bit0..bit6 = segments A..G, bit7 = decimal point
const uint8_t SEVENSEG_FONT[10] = {
  0x3F, 0x06, 0x5B, 0x4F, 0x66, 0x6D, 0x7D, 0x07, 0x7F, 0x6F
};
const uint8_t SEVENSEG_DASH = 0x40;  // segment G only
const uint8_t SEVENSEG_DP   = 0x80;  // trailing dot, marks a slot as showing a setpoint
const uint8_t SEVENSEG_O    = 0x3F;  // same as 0 — used for ON / OF
const uint8_t SEVENSEG_N    = 0x37;  // A B C E F
const uint8_t SEVENSEG_F    = 0x71;  // A E F G

bool sevenSegPresent = false;
uint8_t sevenSegFailStreak = 0;
unsigned long sevenSegLastSample = 0;
unsigned long sevenSegLastWrite = 0;
unsigned long sevenSegLastProbe = 0;

// Mirror of what the panel is currently showing, so we only touch the bus on a change.
uint8_t sevenSegShadow[SEVENSEG_DIGITS];
bool sevenSegShadowValid = false;

bool  sevenSegBaselined = false;   // setpoint watch armed on the first loop pass
float sevenSegSeenTempSet = 0;
float sevenSegSeenHumSet = 0;
bool sevenSegTempHolding = false;
bool sevenSegHumHolding = false;
unsigned long sevenSegTempSetAt = 0;
unsigned long sevenSegHumSetAt = 0;

bool sevenSegCmd(uint8_t cmd) {
  Wire.beginTransmission(SEVENSEG_ADDR);
  Wire.write(cmd);
  return Wire.endTransmission() == 0;
}

// Oscillator on, display on / blink off, full brightness.
bool sevenSegInit() {
  sevenSegPresent = sevenSegCmd(0x21) && sevenSegCmd(0x81) && sevenSegCmd(0xEF);
  sevenSegShadowValid = false;  // panel RAM state unknown, force a redraw
  if (sevenSegPresent) sevenSegFailStreak = 0;
  return sevenSegPresent;
}

// HT16K33 RAM is 16 bits per row; these panels only drive the low byte.
// Retry a couple of times: a single NAK (WiFi RF / busy sensor on the same bus)
// must not freeze the last frame on the glass.
bool sevenSegWriteDigits(const uint8_t* seg) {
...
// Two zero-padded digits. A setpoint gets one trailing dot on the right-hand digit
// ("22.") to tell it apart from a live reading; live readings carry no dot.
// NaN or out of 0..99 shows "--".
void sevenSegPair(float value, uint8_t* out, bool isSetpoint) {
  uint8_t dp = isSetpoint ? SEVENSEG_DP : 0;
  long v = isnan(value) ? -1 : lroundf(value);
  if (v < 0 || v > 99) {
    out[0] = SEVENSEG_DASH;
    out[1] = SEVENSEG_DASH | dp;
    return;
  }
  out[0] = SEVENSEG_FONT[v / 10];
  out[1] = SEVENSEG_FONT[v % 10] | dp;
}

// Last two sections: 10 / 20 / 40, or "--" when the system or fan is off.
void sevenSegFanPair(uint8_t* out) {
  if (!runState || fanSpeed == FAN_OFF) {
    out[0] = SEVENSEG_DASH;
    out[1] = SEVENSEG_DASH;
    return;
  }
  float code = NAN;
  switch (fanSpeed) {
    case FAN_LOW:  code = 20; break;
    case FAN_MED:  code = 30; break;
    case FAN_HIGH: code = 40; break;
    default: break;
  }
  sevenSegPair(code, out, false);
}

// Digits [4-5] when there is no SDP810: ON / OF from runState.
void sevenSegRunPair(uint8_t* out) {
  out[0] = SEVENSEG_O;
  out[1] = runState ? SEVENSEG_N : SEVENSEG_F;
}

// All dashes, so the installer sees the panel is alive before the first reading lands.
void sevenSegShowDashes() {
  if (!sevenSegPresent) return;
  uint8_t seg[SEVENSEG_DIGITS];
  for (uint8_t i = 0; i < SEVENSEG_DIGITS; i++) seg[i] = SEVENSEG_DASH;
  sevenSegWriteDigits(seg);
}

// Call once per loop(). Non-blocking: samples every SEVENSEG_SAMPLE_MS and only spends
// ~2 ms of I2C when the digits actually change.
void updateSevenSegDisplay() {
  unsigned long now = millis();

  // Probe a missing panel in the background, but NEVER skip the hold timer —
  // otherwise a single failed I2C write freezes the last setpoint on the glass.
  if (!sevenSegPresent) {
    if (now - sevenSegLastProbe >= SEVENSEG_PROBE_MS) {
      sevenSegLastProbe = now;
      if (sevenSegInit()) {
        sevenSegFailStreak = 0;
        Serial.println("✓ 7-segment display (re)connected at I2C 0x70");
      }
    }
  }

  if (now - sevenSegLastSample < SEVENSEG_SAMPLE_MS) return;
  sevenSegLastSample = now;

  // Adopt the current setpoints as the baseline on the first pass, so restoring them
  // from NVS during boot isn't mistaken for a user adjustment.
  if (!sevenSegBaselined) {
    sevenSegBaselined = true;
    sevenSegSeenTempSet = tempSet;
    sevenSegSeenHumSet = humSet;
  }

  // Re-arm on every change, so spamming +/- keeps the target on screen.
  if (fabsf(tempSet - sevenSegSeenTempSet) > 0.05f) {
    sevenSegSeenTempSet = tempSet;
    sevenSegTempSetAt = now;
    sevenSegTempHolding = true;
  }
  if (fabsf(humSet - sevenSegSeenHumSet) > 0.05f) {
    sevenSegSeenHumSet = humSet;
    sevenSegHumSetAt = now;
    sevenSegHumHolding = true;
  }

  // Unsigned elapsed compare, so the hold is correct across the millis() rollover.
  if (sevenSegTempHolding && (now - sevenSegTempSetAt) >= SEVENSEG_SET_HOLD_MS) {
    sevenSegTempHolding = false;
  }
  if (sevenSegHumHolding && (now - sevenSegHumSetAt) >= SEVENSEG_SET_HOLD_MS) {
    sevenSegHumHolding = false;
  }

  uint8_t seg[SEVENSEG_DIGITS];
  sevenSegPair(sevenSegTempHolding ? tempSet : filtTempC, &seg[0], sevenSegTempHolding);
  sevenSegPair(sevenSegHumHolding  ? humSet  : filtHum,   &seg[2], sevenSegHumHolding);
  if (useSDP810) {
    sevenSegPair(fabsf(sdp810_pressure), &seg[4], false);
  } else {
    sevenSegRunPair(&seg[4]);
  }
  sevenSegFanPair(&seg[6]);

  if (!sevenSegPresent) return;

  bool changed = !sevenSegShadowValid || memcmp(seg, sevenSegShadow, SEVENSEG_DIGITS) != 0;
  if (!changed && (now - sevenSegLastWrite) < SEVENSEG_KEEPALIVE_MS) return;

  if (sevenSegWriteDigits(seg)) {
    memcpy(sevenSegShadow, seg, SEVENSEG_DIGITS);
    sevenSegShadowValid = true;
    sevenSegLastWrite = now;
    sevenSegFailStreak = 0;
  } else {
    // A lone NAK is common when WiFi and the SHT45 share the bus. Only drop the
    // panel after several consecutive failures so the 2.5s flash can still end.
    if (sevenSegFailStreak < 255) sevenSegFailStreak++;
    if (sevenSegFailStreak >= 3) {
      sevenSegPresent = false;
      sevenSegShadowValid = false;
      sevenSegLastProbe = now;
    }
  }
}
...
void recoverI2CBus() {
  ...
  // The 7-segment panel shares this bus, so re-apply its init with the sensors.
  sevenSegInit();
...
void messageHandler(char* topic, byte* payload, unsigned int length)
{
  ...
  // Device-originated reports must never be treated as commands. If AWS/local
  // ever loops telemetry/state back onto the command topic, applying m1_start /
  // fan / setpoints from those payloads would NVS-write every 2s and re-arm the
  // 7-seg setpoint flash forever.
  {
    const char* msgType = doc["type"] | "";
    if (msgType[0] && (!strcmp(msgType, "telemetry") || !strcmp(msgType, "state") || !strcmp(msgType, "log"))) {
      return;
    }
  }
  ...
  if (doc.containsKey("setpoint")){
    ...
    if (sp >= 1 && sp <= 100 && fabsf(sp - tempSet) > 0.05f){
      tempSet = sp;
  ...
  if (doc.containsKey("humset")){
    float hs = doc["humset"];
    if (hs >= 10 && hs <= 90 && fabsf(hs - humSet) > 0.05f){
      humSet = hs;
      pendingAwsPrefWrite = true;
...
void ensureMqtt(){
  ...
  espNet.setTimeout(2);  // Cap blocking TCP reads on underlying client
...
//  tempSet / humSet / setFanSpeed. Nothing else in the firmware is modified.
//
//  Decode runs on a dedicated task so a blocking network call in loop()
//  can never delay the receiver. The task only records intent; the main
//  loop applies it via applyPendingIr() (PubSubClient is not re-entrant).
//
//  Humidity uses the remote's TIMER ON / TIMER OFF buttons (the ones labeled
//  ON and OFF, not the power button). Those live in Daikin216 padding bytes
//  18-20, which toCommon() ignores, so we diff the raw frame:
//    TIMER ON  (bytes 19/20) -> humidity -5%
//    TIMER OFF (byte 18)     -> humidity +5%
// =====================================================================
...
static const float    IR_HUM_MIN    = 10.0f;  // same band the MQTT/AWS handlers accept
static const float    IR_HUM_MAX    = 90.0f;
static const float    IR_HUM_STEP   = 5.0f;   // one TIMER ON/OFF press

// Daikin216 timer ON/OFF hide in the library's pad2[3] (bytes 18-20).
static const uint8_t  IR_TIMER_OFF_BYTE = 18;  // TIMER OFF  -> humidity up
static const uint8_t  IR_TIMER_ON_B1    = 19;  // TIMER ON   -> humidity down
static const uint8_t  IR_TIMER_ON_B2    = 20;
...
struct IrRemoteState {
  ...
  uint8_t fan   = 0;
  uint8_t mode  = 0;
  uint8_t timerOff = 0;  // raw byte 18
  uint8_t timerOn1 = 0;  // raw byte 19
  uint8_t timerOn2 = 0;  // raw byte 20
...
struct IrPending {
  ...
  uint8_t fanRemote = 0;
  bool    doHum   = false;
  float   humDelta = 0.0f;  // accumulated TIMER ON/OFF steps until the loop applies them
};
...
// True when the IR task has queued an action the main loop has not applied yet.
static bool irHasPending() {
  portENTER_CRITICAL(&g_irMux);
  bool has = g_irPending.has;
...
static uint8_t irBitCount(uint8_t v) {
  uint8_t n = 0;
  while (v) { n = (uint8_t)(n + (v & 1u)); v = (uint8_t)(v >> 1); }
  return n;
}

// TIMER ON -> -step, TIMER OFF -> +step. Same prefs/log path as MQTT humidity.
void irAdjustHum(float delta) {
  float newSet = humSet + delta;
  if (newSet < IR_HUM_MIN) newSet = IR_HUM_MIN;
...
static bool irDecodeStep() {
  ...
  // Copy timer bytes before resume() — the decoder buffer is reused.
  uint8_t tOff = 0, tOn1 = 0, tOn2 = 0;
  if (irResults.state != nullptr) {
    tOff = irResults.state[IR_TIMER_OFF_BYTE];
    tOn1 = irResults.state[IR_TIMER_ON_B1];
    tOn2 = irResults.state[IR_TIMER_ON_B2];
  }
  ...
  if (!irLast.valid) {
    ...
    irLast.timerOff = tOff; irLast.timerOn1 = tOn1; irLast.timerOn2 = tOn2;
  ...
  bool  chgMode  = (mode    != irLast.mode);
  bool  primaryChanged = chgPower || chgTemp || chgFan || chgMode;
  ...
  // TIMER ON / TIMER OFF (labeled ON / OFF on the remote) -> humidity +/-.
  // These buttons increment a clock field, so the XOR mask is different every
  // press; we only care which byte region moved. Skip if a primary AHU field
  // also changed — that press was power/temp/fan/mode, not a timer key.
  float humDelta = 0.0f;
  if (!primaryChanged) {
    uint8_t offBits = irBitCount((uint8_t)(tOff ^ irLast.timerOff));
    uint8_t onBits  = (uint8_t)(irBitCount((uint8_t)(tOn1 ^ irLast.timerOn1)) +
                                irBitCount((uint8_t)(tOn2 ^ irLast.timerOn2)));
    if (onBits > offBits)      humDelta = -IR_HUM_STEP;
    else if (offBits > onBits) humDelta = +IR_HUM_STEP;
  }

  irLast.power = st.power; irLast.tempC = tempC; irLast.fan = fan; irLast.mode = mode;
  irLast.timerOff = tOff; irLast.timerOn1 = tOn1; irLast.timerOn2 = tOn2;

  if (!chgPower && !chgTemp && !chgFan && humDelta == 0.0f) return true;  // nothing actionable

  // Merge into the pending snapshot (latest press wins per field; humidity steps accumulate).
  portENTER_CRITICAL(&g_irMux);
  ...
  if (humDelta != 0.0f) {
    g_irPending.doHum = true;
    g_irPending.humDelta += humDelta;
  }
  portEXIT_CRITICAL(&g_irMux);
  return true;
}
...
void applyPendingIr() {
  bool  doPower, powerOn, doTemp, doFan, doHum;
  float tempVal, humDelta;
  uint8_t fanRemote;
  ...
  doHum     = g_irPending.doHum;     humDelta  = g_irPending.humDelta;
  g_irPending.has = false;
  g_irPending.doPower = g_irPending.doTemp = g_irPending.doFan = g_irPending.doHum = false;
  ...
  // TIMER ON / TIMER OFF -> humidity +/- (same prefs/log path as MQTT humidity).
  if (doHum) {
...
void setup()
{
  ...
  // 7-segment panel: init before sensor detection so it lights up early in boot.
  if (sevenSegInit()) {
    sevenSegShowDashes();
    Serial.println("✓ 7-segment display initialized (0x70): [temp][humidity][diffP or ON/OF][fan]");
  } else {
    Serial.println("⚠️ 7-segment display not found at 0x70 (will retry in loop)");
  }
...
void loop()
{
  ...
  // Refresh the 7-segment panel. Placed before the shutdown branch (which returns
  // early) so the digits keep updating during the shutdown sequence too.
  updateSevenSegDisplay();
