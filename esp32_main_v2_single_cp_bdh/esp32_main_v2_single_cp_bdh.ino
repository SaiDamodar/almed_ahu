...
#define BUILD_VERSION "v2.6.2-SCP-BDH"
#define BUILD_DATE "2026-06-16"
#define BUILD_FEATURES "single-CP BDH, no-auto-start-on-power-restore, shared-topic-cmd-guard"
...
// ⚠️ BDH WARNING: This asset is the ACTIVE-HIGH (dual-CP, auto-start) firmware. If you push an
// OTA "latest" to this BDH unit with this asset name, it will be OVERWRITTEN by the active-high
// build and lose all single-CP/auto-resume behavior. Before enabling OTA on BDH units, upload a
// dedicated BDH .bin to the GitHub release and set its exact name here.
...
CpMode cpMode = CP_SINGLE;  // BDH: single CP only — never auto-switch
...
const char* SITE = "SIR";    // Kaveri Hospital
const char* ROOM = "OFFICE";         // Burns Ward
...
unsigned long g_setupCompleteMs = 0;  // Set at end of setup(); boot-window retained-msg guard
...
void restoreSystemState(){
  ...
  if (saveTime > 0 && now < 300000) {
    ...
    cpMode = (CpMode)prefs.getInt("cpMode", CP_SINGLE);  // BDH: default single
    ...
    if (wasRunning && !wasShuttingDown) {
      // AUTO-RESUME after an UNEXPECTED reset / brief power dip so the AHU recovers on its
      // own and does NOT require an MCB trip. A deliberate STOP calls clearSystemState()
      // which sets runState=false in NVS, so this branch only runs when the unit was
      // genuinely running before the reset. Cross-AHU stray commands are blocked separately
      // by the shared-topic target_thing guard in messageHandler().
      pendingRecoveryStart = true;
      runState = false;  // becomes true at end of setup() once init is complete

      // Power the system master relay so fans/ozone/etc. come back online.
      systemWrite(true);
      Serial.println("✓ System relay ON (auto-resume after reset)");

      // BDH single-CP safety: force CP1 as the active compressor.
      if (cpMode == CP_SINGLE) cpActive = 1;

      // Start compressors + heater OFF and let controlCP()/controlHeater() re-decide from
      // live temperature/humidity. This avoids slamming the compressor ON the instant power
      // returns (the cycle delay below protects it further).
      cpOn = false;
      cp2On = false;
      dualCpBothOn = false;
      heatOn = false;
      ...
      // Restore fan to its saved speed (system is running again).
      ...
      // Compressor protection after a power event: force a full cycle delay before the CP
      // can re-engage, regardless of saved timing. controlCP() will turn it on when due.
      cpLastOffAt = now;
      ...
      Serial.println("✓ AUTO-RESUME after reset: system relay ON, CP/heater will re-engage by sensor");
      Serial.print("  CP mode: "); Serial.print(cpMode == CP_DUAL_AUTO ? "DUAL" : "SINGLE");
      Serial.print(" | Active CP: CP"); Serial.print(cpActive);
...
void messageHandler(char* topic, byte* payload, unsigned int length)
{
  ...
  // BDH FIX: On the shared esp32/sub topic any control command must carry
  // target_thing (or thing) matching THINGNAME. Device-specific topic skips this check.
  bool _cmdMine;
  {
    String _ct(topic);
    bool _shared = (_ct == String(AWS_IOT_SUBSCRIBE_TOPIC));
    if (_shared) {
      String _tgt;
      if (doc.containsKey("target_thing")) _tgt = doc["target_thing"].as<String>();
      else if (doc.containsKey("thing"))   _tgt = doc["thing"].as<String>();
      _cmdMine = (_tgt == String(THINGNAME));
      if (!_cmdMine) {
        Serial.printf("⚠️ CMD ignored (shared topic): for '%s', this is '%s'\n",
                      _tgt.c_str(), THINGNAME);
      }
    } else {
      _cmdMine = true;
    }
  ...
  // Handle CP mode switching (dual/single) via cloud — guarded by target_thing on shared topic
  if (_cmdMine && doc.containsKey("cpMode")){
  ...
  // Handle CP active selection (1 or 2) via cloud — guarded by target_thing on shared topic
  if (_cmdMine && doc.containsKey("cpActive")){
...
void setup()
{
  ...
  // Load CP mode (BDH: default to single CP)
  cpMode = (CpMode)prefs.getInt("cpMode", CP_SINGLE);
  cpActive = prefs.getInt("cpActive", 1);
  cpLastSwitchAt = prefs.getULong("cpLastSwitchAt", 0);
  // BDH single-CP safety: only CP1 is wired, so never let a stale NVS value select CP2.
  if (cpMode == CP_SINGLE && cpActive != 1) {
    cpActive = 1;
    prefs.putInt("cpActive", cpActive);
  }
  ...
  // Auto-resume: if the unit was genuinely running before an unexpected reset/power dip,
  // bring it back up automatically so no MCB trip / manual restart is needed. A deliberate
  // STOP clears runState in NVS, so this will NOT fire after an operator stop.
  if (pendingRecoveryStart) {
    pendingRecoveryStart = false;
    runState = true;
    motorLogMsg("✓ AUTO-RESUME: AHU was running before reset — resumed automatically");
  }
